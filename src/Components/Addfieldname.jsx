...
const FieldNames = () => {
  ...
  const fetchFields = async () => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_API_URL}field-names`
      );
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmDelete.isConfirmed) {
      ...
      axios
        .delete(`${import.meta.env.VITE_API_URL}field-names/${id}`)
        .then(() => {
  ...
  const handleSave = async (id) => {
    try {
      await axios.patch(`${import.meta.env.VITE_API_URL}field-names/${id}`, {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${import.meta.env.VITE_API_URL}field-names`, formData);
