...
const FieldNames = () => {
  ...
  const fetchFields = async () => {
    try {
      const response = await axios.get(
        "http://3.6.61.72:3000/field-names"
      );
  ...
  const handleDelete = async (id) => {
    ...
    if (confirmDelete.isConfirmed) {
      ...
      axios
        .delete(`http://3.6.61.72:3000/field-names/${id}`)
        .then(() => {
  ...
  const handleSave = async (id) => {
    try {
      await axios.patch(`http://3.6.61.72:3000/field-names/${id}`, {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.post("http://3.6.61.72:3000/field-names", formData);
