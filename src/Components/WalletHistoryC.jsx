  ...
  const filteredRecords = walletHistory.filter((record) => {
    ...
    const matchesSearch =
      ...
      (record.merchantOrderId &&
        record.merchantOrderId.toLowerCase().includes(lowerCaseSearchQuery)) ||
      (record.transactionId &&
        record.transactionId.toLowerCase().includes(lowerCaseSearchQuery)) ||
      (record.type && record.type.toLowerCase().includes(lowerCaseSearchQuery)) ||
      (record.status && record.status.toLowerCase().includes(lowerCaseSearchQuery)) ||
      (record.id && record.id.toString().includes(lowerCaseSearchQuery)) ||
      (record.amount && record.amount.toString().includes(lowerCaseSearchQuery)) ||
      (record.paymentDetails &&
        JSON.stringify(record.paymentDetails).toLowerCase().includes(lowerCaseSearchQuery));

    return matchesType && matchesSearch;
  });

  const sortedRecords = filteredRecords.sort((a, b) => {
    // Ensure dates are valid for comparison, fallback to 0 if null/undefined
