...
export default function CustomerDashboard() {
  ...
  useEffect(() => {
    ...
    // 1. Applied / Completed
    axios
      .get(`http://3.6.61.72:3000/userdashboard/total-applied/${userId}`)
      .then((res) => setAppliedCount(res.data.totalCount))
      .catch(console.error);

    axios
      .get(`http://3.6.61.72:3000/userdashboard/total-completed/${userId}`)
      .then((res) => setCompletedCount(res.data.totalCompleted))
      .catch(console.error);

    // 2. Wallet balance
    axios
      .get(`http://3.6.61.72:3000/wallet`, { headers: authHeaders })
      .then((res) => {
    ...
    axios
      .get(`http://3.6.61.72:3000/userdashboard/status-count/${userId}`)
      .then((res) =>
        setStatusData(
          res.data.map((item) => ({
            status: item.status,
    ...
    axios
      .get(`http://3.6.61.72:3000/userdashboard/category-counts/${userId}`)
      .then((res) => {
    ...
    // 5. Notifications
    axios
      .get("http://3.6.61.72:3000/notifications/active")
  ...
  const fetchCategories = async () => {
    try {
      const { data } = await axios.get("http://3.6.61.72:3000/categories");
      setCategories(data);
    } catch {}
  };
  const fetchSubcategories = async () => {
    try {
      const { data } = await axios.get(
        "http://3.6.61.72:3000/subcategories"
      );
  ...
  const fetchPrices = async () => {
    try {
      const { data } = await axios.get("http://3.6.61.72:3000/prices");
  ...
  const openDocsModal = async (catId, subId) => {
    try {
      const { data } = await axios.get(
        `http://3.6.61.72:3000/required-documents/${catId}/${subId}`
      );
