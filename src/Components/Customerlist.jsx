...
const CustomerList = () => {
  ...
  const apiUrl = "http://3.6.61.72:3000/users/customers";
  ...
  const handleUpdateCustomer = async (id) => {
    try {
      if (updatedPassword) {
        await axios.patch(`http://3.6.61.72:3000/users/password/${id}`, {
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      ...
      await axios.patch(`http://3.6.61.72:3000/users/status/${id}`, {
        status: newStatus,
      });
