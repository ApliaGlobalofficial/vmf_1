...
const ClistPage = () => {
  ...
  useEffect(() => {
    if (categoryId) {
      axios
        .get(`http://3.6.61.72:3000/categories/${categoryId}`)
        .then((res) => setCategoryName(res.data.category_name))
        .catch((err) => console.error("Error loading category:", err));
    }
    if (subcategoryId) {
      axios
        .get(`http://3.6.61.72:3000/subcategories/${subcategoryId}`)
  ...
  useEffect(() => {
    ...
    axios
      .get("http://3.6.61.72:3000/documents/list")
      .then((resp) => {
  ...
  useEffect(() => {
    if (userId && categoryId && subcategoryId) {
      const url = `http://3.6.61.72:3000/documents/doc/${categoryId}/${subcategoryId}/${userId}`;
      axios
        .get(url)
  ...
  const handleViewCertificate = async (docId) => {
    ...
    try {
      const resp = await axios.get(
        `http://3.6.61.72:3000/certificates/${cert.certificate_id}`
      );
