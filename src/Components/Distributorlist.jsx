...
const DistributorList = () => {
  ...
  const handleInputChange = async (e) => {
    ...
    // Validate the field in real-time, except shopAddress (optional)
    if (name !== "shopAddress") {
      const newErrors = await validateField(name, value);
      setFormData((prev) => ({
        ...prev,
  ...
  const handleUpdateDistributor = async (id) => {
    try {
      ...
      setDistributors((prev) =>
        prev.map((d) =>
          d.user_id === id ? { ...d, password: updatedPassword } : d
        )
      );
  ...
  // Prepare the list to display based on activeTab
  const visibleDistributors =
    activeTab === "all"
      ? distributors
      : distributors.filter(
          (d) => d.edit_request_status && d.edit_request_status !== ""
        );
            ...
            <table className="w-full border border-[#776D6DA8] text-sm bg-white shadow-md rounded-md">
              ...
              <tbody>
                {visibleDistributors.length > 0 ? (
                  visibleDistributors.map((distributor, idx) => {
                    const isEditingRow = editingId === distributor.user_id;

                    // Highlight entire row if:
                    //  • currently editing → light blue
                    //  • else if “Edit Requests” tab → light yellow
                    //  • otherwise → default with hover-orange
                    ...
                    // Password cell highlight: blue ring if editing, yellow bg if in Edit Requests tab
                    ...
                    return (
                      <tr
                        ...
                        className={`${rowHighlight} ${
                          idx % 2 === 0 ? "" : "" /* striped color no longer needed because rowHighlight covers it */
                        } border`}
                      >
                        <td className="px-4 py-3 border border-[#776D6DA8] text-center">
                          {distributor.user_id}
                        ...
                        <td
                          className={`px-4 py-3 border border-[#776D6DA8] text-center ${passwordCellHighlight}`}
                        >
                          {isEditingRow ? (
                            <input
                              type="text"
                        ...
                        <td className="px-4 py-3 border border-[#776D6DA8] text-center">
                          {Array.isArray(distributor.user_documents) ? (
                            distributor.user_documents.map((doc, i) => (
                              <div key={i}>
                                <a
                                  href={doc.file_path}
                                  target="_blank"
                  ...
                  <tr>
                    <td
                      colSpan="12"
                      className="px-4 py-3 border border-[#776D6DA8] text-center text-gray-500 italic"
                    >
                      {activeTab === "all"
                        ? "No distributors found."
