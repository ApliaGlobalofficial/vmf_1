...
const ErrorRequests = () => {
  ...
  const fetchAssignedDocuments = async () => {
    try {
      const { data } = await axios.get(
        `http://3.6.61.72:3000/documents/list/${distributorId}`
      );
  ...
  const handleDownloadReceipt = async (applicationId) => {
    try {
      const { data } = await axios.get(
        `http://3.6.61.72:3000/documents/receipt/${applicationId}`
      );
  ...
  const handleDownloadCertificate = async (applicationId) => {
    try {
      const { data } = await axios.get(
        `http://3.6.61.72:3000/certificates/certificate/${applicationId}`
      );
  ...
  const handleUpload = async (req, type) => {
    ...
    const url =
      type === "certificate"
        ? `http://3.6.61.72:3000/certificates/update/${req.document_id}`
        : `http://3.6.61.72:3000/documents/update-receipt/${req.document_id}`;
    const method = type === "certificate" ? "patch" : "put";

    try {
      await axios[method](url, formData);
      await axios.patch(
        `http://3.6.61.72:3000/request-errors/update-status/${req.request_id}`,
        {
          request_status:
