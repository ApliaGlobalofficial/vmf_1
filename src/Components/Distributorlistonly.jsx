...
const UserTable = () => {
  ...
  const apiUrl = "http://3.6.61.72:3000/users/distributors";
  ...
  const checkEmailExists = async (email) => {
    try {
      const response = await axios.get(
        `http://3.6.61.72:3000/users/check-email/${email}`,
        { timeout: 30000 }
      );
  ...
  const handleUpdateDistributor = async (id) => {
    try {
      if (updatedPassword) {
        await axios.patch(
          `http://3.6.61.72:3000/users/password/${id}`,
  ...
  const handleDeleteDistributor = async (id) => {
    ...
    if (confirmDelete.isConfirmed) {
      ...
      try {
        await axios.delete(`http://3.6.61.72:3000/users/delete/${id}`, {
          timeout: 30000,
        });
  ...
  const handleStatusChange = async (id, newStatus) => {
    try {
      ...
      await axios.patch(
        `http://3.6.61.72:3000/users/status/${id}`,
        { status: newStatus },
        { timeout: 30000 }
      );
  ...
  const updateEditRequestStatus = async (id, newStatus) => {
    try {
      await axios.patch(`http://3.6.61.72:3000/users/request-edit/${id}`, {
        status: newStatus,
      });
